...
async function initializeSchema() {
  ...
  try {
    ...
    const requiredTables = ['accounts', 'comments', 'reactions', 'questions', 'answers', 'verse_suggestions', 'favorites', 'push_subscriptions', 'user_history']
    ...
    if (missingTables.length === 0) {
      ...
      await pool.query('alter table accounts add column if not exists verse_interval_hours integer default 2')
