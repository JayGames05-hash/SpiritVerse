const jsonHeaders = { 'Content-Type': 'application/json' }
const authListeners = new Set()

function emitAuthEvent(event, session) {
  authListeners.forEach((listener) => {
    try {
      listener.callback(event, session)
    } catch (error) {
      console.error('auth event failed', error)
    }
  })
}

async function request(path, options = {}) {
  try {
    const response = await fetch(path, {
      credentials: 'include',
      ...options,
      headers: {
        ...jsonHeaders,
        ...(options.headers || {}),
      },
    })

    const body = await response.json().catch(() => null)
    if (!response.ok) {
      return { data: null, error: { message: body?.error || response.statusText } }
    }

    return { data: body, error: null }
  } catch (error) {
    return { data: null, error: { message: error.message || 'Network error' } }
  }
}

const auth = {
  getUser: async () => {
    const { data, error } = await request('/api/auth/user', { method: 'GET' })
    if (error) return { data: { user: null }, error }
    return { data: { user: data.user }, error: null }
  },
  signUp: async ({ email, password }, { options } = {}) => {
    const saintName = options?.data?.saint_name || ''
    const body = { email, password }
    if (saintName) body.saint_name = saintName
    const { data, error } = await request('/api/auth/signup', {
      method: 'POST',
      body: JSON.stringify(body),
    })
    if (!error) {
      emitAuthEvent('SIGNED_IN', { user: data.user })
    }
    return { data: { user: data?.user ?? null }, error }
  },
  signInWithPassword: async ({ email, password }) => {
    const { data, error } = await request('/api/auth/signin', {
      method: 'POST',
      body: JSON.stringify({ email, password }),
    })
    if (!error) {
      emitAuthEvent('SIGNED_IN', { user: data.user })
    }
    return { data: { user: data?.user ?? null }, error }
  },
  signOut: async () => {
    const { data, error } = await request('/api/auth/signout', { method: 'POST' })
    if (!error) {
      emitAuthEvent('SIGNED_OUT', { user: null })
    }
    return { data, error }
  },
  onAuthStateChange: (callback) => {
    const listener = { callback }
    authListeners.add(listener)
    return {
      data: {
        subscription: {
          unsubscribe() {
            authListeners.delete(listener)
          },
        },
      },
    }
  },
}

export async function getComments(postId) {
  const { data, error } = await request(`/api/comments?post_id=${encodeURIComponent(postId)}`, { method: 'GET' })
  return { data: data?.comments ?? [], error }
}

export async function createComment(postId, text, authorName) {
  const { data, error } = await request('/api/comments', {
    method: 'POST',
    body: JSON.stringify({ post_id: postId, text, author_name: authorName }),
  })
  return { data: data?.comment ?? null, error }
}

export async function updateComment(id, values) {
  const { data, error } = await request(`/api/comments/${encodeURIComponent(id)}`, {
    method: 'PUT',
    body: JSON.stringify(values),
  })
  return { data: data?.comment ?? null, error }
}

export async function deleteComment(id) {
  const { data, error } = await request(`/api/comments/${encodeURIComponent(id)}`, {
    method: 'DELETE',
  })
  return { data, error }
}

export async function getReactions(postId) {
  const { data, error } = await request(`/api/reactions?post_id=${encodeURIComponent(postId)}`, { method: 'GET' })
  return {
    data: {
      count: data?.count ?? 0,
      liked: data?.liked ?? false,
      reactions: data?.reactions ?? [],
    },
    error,
  }
}

export async function addReaction(postId) {
  const { data, error } = await request('/api/reactions', {
    method: 'POST',
    body: JSON.stringify({ post_id: postId, type: 'like' }),
  })
  return { data: data?.reaction ?? null, error }
}

export async function removeReaction(postId) {
  const { data, error } = await request('/api/reactions', {
    method: 'DELETE',
    body: JSON.stringify({ post_id: postId }),
  })
  return { data, error }
}

export async function getAdminComments() {
  const { data, error } = await request('/api/comments', { method: 'GET' })
  return { data: data?.comments ?? [], error }
}

export async function updateCommentStatus(id, status) {
  const { data, error } = await request(`/api/comments/${encodeURIComponent(id)}`, {
    method: 'PUT',
    body: JSON.stringify({ status }),
  })
  return { data: data?.comment ?? null, error }
}

export async function getVerseSuggestions(status) {
  const statusQuery = status ? `?status=${encodeURIComponent(status)}` : ''
  const { data, error } = await request(`/api/verses/suggestions${statusQuery}`, { method: 'GET' })
  return { data: data?.suggestions ?? [], error }
}

export async function updateVerseSuggestionStatus(id, status) {
  const { data, error } = await request(`/api/verses/${encodeURIComponent(id)}`, {
    method: 'PUT',
    body: JSON.stringify({ status }),
  })
  return { data: data?.suggestion ?? null, error }
}

export async function getFavorite(postId) {
  const { data, error } = await request(`/api/favorites?post_id=${encodeURIComponent(postId)}`, { method: 'GET' })
  return { data: data?.favorite ?? false, error }
}

export async function getFavorites() {
  const { data, error } = await request('/api/favorites', { method: 'GET' })
  return { data: data?.favorites ?? [], error }
}

export async function addFavorite(postId) {
  const { data, error } = await request('/api/favorites', {
    method: 'POST',
    body: JSON.stringify({ post_id: postId }),
  })
  return { data: data?.favorite ?? null, error }
}

export async function removeFavorite(postId) {
  const { data, error } = await request('/api/favorites', {
    method: 'DELETE',
    body: JSON.stringify({ post_id: postId }),
  })
  return { data: data?.ok ?? false, error }
}

export async function getHistory() {
  const { data, error } = await request('/api/history', { method: 'GET' })
  return { data: data?.history ?? [], error }
}

export async function logHistory(postId) {
  const { data, error } = await request('/api/history', {
    method: 'POST',
    body: JSON.stringify({ post_id: postId }),
  })
  return { data: data?.entry ?? null, error }
}

export async function getReflection(postId) {
  const { data, error } = await request(`/api/reflections?post_id=${encodeURIComponent(postId)}`, { method: 'GET' })
  return { data: data?.reflection ?? null, error }
}

export async function getReflections() {
  const { data, error } = await request('/api/reflections', { method: 'GET' })
  return { data: data?.reflections ?? [], error }
}

export async function saveReflection(postId, note) {
  const { data, error } = await request('/api/reflections', {
    method: 'POST',
    body: JSON.stringify({ post_id: postId, note }),
  })
  return { data: data?.reflection ?? null, error }
}

export async function deleteReflection(postId) {
  const { data, error } = await request('/api/reflections', {
    method: 'DELETE',
    body: JSON.stringify({ post_id: postId }),
  })
  return { data: data?.ok ?? false, error }
}

export async function getAdminMetrics() {
  const { data, error } = await request('/api/admin/metrics', { method: 'GET' })
  return { data: data?.metrics ?? null, error }
}

export { auth }
