...
export async function getUserFromRequest(req) {
  ...
  if (process.env.DATABASE_URL) {
    const result = await query(
      `select id, email, saint_name, full_name, is_admin, verse_interval_minutes, notification_schedule, created_at
       from accounts
...
export async function createSession(res, userId) {
  ...
  let userRow = null
  try {
    const r = await query('select id, email, saint_name, full_name, is_admin, verse_interval_minutes, notification_schedule, created_at from accounts where id = $1', [userId])
    userRow = r.rows[0]
  } catch (err) {
...
export function serializeUser(userRow) {
  ...
  return {
    ...
    notification_schedule: userRow.notification_schedule || 'every_2_hours',
